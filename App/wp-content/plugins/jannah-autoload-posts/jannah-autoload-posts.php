...
class JANNAH_AUTOLOAD_POSTS {
	...
	private function define_constants() {
		...
		define( 'JANNAH_AUTOLOAD_POSTS_VERSION', '1.0.2' );
	}
	...
	function insert_iframe_js_codes(){
		?>
		<script data-cfasync="false" type="text/javascript" src="<?php echo JANNAH_AUTOLOAD_POSTS_URL ?>js/autoload-parent.js"></script>
		<?php
	}
	...
	function get_next_iframe(){
		...
		if ( ! empty( $next_posts ) ) {
			...
			echo '
				<script data-cfasync="false">var tieAutoLoadPosts = '. wp_json_encode( $next_posts ) .';</script>
	...
	function child_head(){
		...
		// We don't use wp_enqueue_script to avoid this file to got concatenated by cache plugins
		echo '<script data-cfasync="false" type="text/javascript" src="'. JANNAH_AUTOLOAD_POSTS_URL . 'js/autoload-child.js"></script>';
	...
	function child_footer(){
		...
		if( ! empty( $js_codes ) ){
			echo '<script data-cfasync="false">'.  $js_codes .'</script>';
		}
	}
